 ...
 * the use of a grub screw and nut is optional
 */
gear1_d_shaft = 1;
gear1_setscrew_nut = 1;
...
/* Shaft (hobbed bolt) diameter
 * Be sure to add some tolerance for printer error
 * AndrewBCN : in my case, 0.4mm was way too much, 0.2mm seems OK
 */
gear2_shaft_diameter = 8 + 0.2;
 ...
 *  Now gear2_shaft_height should be equal to:
 *   X - Y - 0.5
 */
/*
 ...
 *   X: 28mm
 *
 * Another black hobbed bolt bought on aliexpress
 *   X: 25.7mm
 *
...
gear2_decoration_flower = 6; //Set to number of petals (6 looks good to me)
...
module gear1() {
    ...
    difference() {
	  ...
	  if (gear1_d_shaft == 1) {
	    translate([-3.6,0,1.5]) cube([3.10, 5.5, 17], center=true);
	  }
        }
        
        if (gear1_setscrew_nut == 1) {
	    //Setscrew shaft
